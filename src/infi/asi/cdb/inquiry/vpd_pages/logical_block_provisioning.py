...
class LogicalBlockProvisioningVPDPageData(Buffer):
    peripheral_device = buffer_field(where=bytes_ref[0:], type=PeripheralDeviceDataBuffer)
    page_code = be_uint_field(where=bytes_ref[1:2])
...
class LogicalBlockProvisioningPageCommand(EVPDInquiryCommand):
    def __init__(self):
        super(BlockLimitsPageCommand, self).__init__(0xb2, 252, buffer_to_struct_adapter(LogicalBlockProvisioningVPDPageData))


__all__ = ["LogicalBlockProvisioningPageCommand", "LogicalBlockProvisioningVPDPageData"]
