...
class ReceiveCopyParametersTestCase(unittest.TestCase):
    ...
    def test_unpack_response(self):
        ...
        self.assertEqual(response.available_data, 42)
        self.assertEqual(response.snlid, 1)
        self.assertEqual(response.max_cscd_descriptor_count, 2)
        self.assertEqual(response.max_segment_descriptor_count, 1)
        self.assertEqual(response.max_descriptor_list_length, 92)
        self.assertEqual(response.max_segment_length, 0)
        self.assertEqual(response.max_inline_data_length, 0)
        self.assertEqual(response.held_data_limit, 0)
        self.assertEqual(response.max_stream_device_transfer_size, 0)
        self.assertEqual(response.total_concurrent_copies, 8)
        self.assertEqual(response.max_concurrent_copies, 8)
        self.assertEqual(response.data_segment_granularity, 16)
        self.assertEqual(response.inline_data_granularity, 16)
        self.assertEqual(response.held_data_granularity, 16)
        self.assertEqual(response.implemented_descriptor_list_length, 2)
        self.assertEqual(response.implemented_descriptor_list, [2, 228])
